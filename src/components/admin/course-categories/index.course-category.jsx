...
export default function CategoryIndex() {
  ...
  useEffect(() => {
    document.title = "course categories - Techarena innovasion "
    getCollections()
  }, [])
  const getCollections = async () => {
    setIsLoading(true);
    await api.get(`course-categories/`).then(({ data }) => {
      setCollections(data)
      setIsLoading(false)
    })
  }
  ...
  const deleteData = async (id) => {
    ...
    await api.delete(`course-categories/${id}`).then(({ data }) => {
      Swal.fire({
        icon: "success",
        text: data.message
      })
  ...
  const columns = [
    { name: "Category", options: { filterOptions: { fullWidth: true } } },
  ...
  const options = {
    ...
    downloadOptions: {
      filename: 'course-categories.csv',
    },
  ...
  const tableHeading = <div className="table-heading"><h1>Course Categories</h1></div>
  ...
  let tableData = []
    ...
    if (collection.length > 0) {
      collection.map((data, index) => {
       return tableData.push([data.category_name, <EditRecord id={data.id}/>, data.id]);
      })
    }
