...
export default function CoursePriceIndex() {
  ...
  useEffect(() => {
    document.title = "course prices - Techarena innovasion "
    getCollections()
  }, [])
  const getCollections = async () => {
    setIsLoading(true);
    await api.get(`course-prices/`).then(({ data }) => {
      typeof data.course_prices === 'undefined' ? setCollections([]) :
        setCollections(data.course_prices)
      setIsLoading(false)
    })
  }
  ...
  const deleteData = async (id) => {
    ...
    await api.delete(`course-prices/${id}`).then(({ data }) => {
      Swal.fire({
        icon: "success",
        text: data.message
      })
  ...
  const options = {
    ...
    downloadOptions: {
      filename: ' course-prices-techarena.csv',
    },
  ...
  const tableHeading = <div className = "table-heading"><h1>Course Prices</h1></div>
  ...
  if (collection.length > 0) {
    collection.map((data, idx) => {
      return tableData.push([data.dept_name,
      data.course_name,
      new Intl.NumberFormat('en-IN', { style:'currency',currency:'NGN'}).format(data.cost),
      <EditRecord id={data.id} />, data.id]);
    })
  }
